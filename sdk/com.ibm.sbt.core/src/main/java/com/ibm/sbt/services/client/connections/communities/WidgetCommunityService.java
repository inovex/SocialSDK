...
public class WidgetCommunityService extends CommunityService {
	...
	/** Class logger */
	private static final String sourceClass = WidgetCommunityService.class.getName();
	...
	public WidgetCommunity createSubCommunity(Community parentCommunity, String title, Collection<String> tags, String content, String type) {
		...
		try {
			...
			if (null == subCommunity) {
				LOG.log(Level.FINE, sMethod + "subcommunity NOT created: " + title);
				LOG.log(Level.FINE, sMethod + "subcommunity NOT created, tags: " + tags);
			} else {
				LOG.log(Level.FINE, sMethod + "subcommunity created: " + subCommunity.getTitle());
				LOG.log(Level.FINE, sMethod + "subcommunity created, tags: " + subCommunity.getTags());
			}
		} catch (Throwable t) {
			// log exception
	...
	private WidgetCommunity prepareCommunityEntity(String title, Collection<String> tags, String content, String type, String uuid, Community parentCommunity) {
		...
		if (null != parentCommunity && null != parentCommunity.getCommunityUuid()) {
			...
			// BAD:  https://my.connections.com/communities/service/html/communityview?communityUuid=8ea4ff45-ef58-4c9b-b131-def2d3e233f7
			// GOOD: https://my.connections.com/communities/service/atom/community/instance?communityUuid=8ea4ff45-ef58-4c9b-b131-def2d3e233f7
...
//			params.put(WIDGET_DEF_ID, widgetDefId.toString());
			...
			if (null != widgetList && widgetList.size() > 0) {
				...
				// TODO Check if a WidgetList entry is complete (or if fields are missing). CommunityList does NOT contain all entry data!
			}
		} catch (Throwable t) {
			// log exception
			LOG.log(Level.WARNING, sMethod + "Exception thrown: " + t);
